...
def format_room_state_header(room_id, func, state_header):
  return align_comments(f'''
; Room ${room_id:04X} state ${func.state_header_addr:04X}: Header
org $8F{func.state_header_addr:04X}
...
dw ${state_header.fx_addr           :04X} ; FX address
dw ${state_header.enemy_pop_addr    :04X} ; Enemy population address (bank $A1)
dw ${state_header.enemy_set_addr    :04X} ; Enemy graphics set address (bank $B4)
dw ${state_header.layer_2_scroll    :04X} ; Layer 2 scroll
dw ${state_header.room_var          :04X} ; Room var
dw ${state_header.room_main_func    :04X} ; Room main routine
dw ${state_header.plm               :04X} ; Room PLM list address
dw ${state_header.library_background:04X} ; Library background
dw ${state_header.room_setup_func   :04X} ; Room setup routine
'''.strip())
...
def main():
  ...
  print(format_room_header(room_id, room_header))

  for func in room_header.state_functions:
    print(format_room_state_function(room_id, func))

  for func in room_header.state_functions:
    state_header = RoomStateHeader.extract(rom, func.state_header_addr)
    print()
    print(format_room_state_header(room_id, func, state_header))
